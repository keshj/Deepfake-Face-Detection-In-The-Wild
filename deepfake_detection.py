...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, file_paths, labels, transform=None):
        assert len(file_paths) == len(labels), "Mismatch between file paths and labels."
...
def compute_mean_and_std(data_dir):
    ...
    """
    cache_file = f"/home/nithira/sp_cup/keshawa/logs/mean_and_std_{os.path.basename(data_dir)}.pth"
...
def get_data_loaders(data_dir, batch_size, transform=None, shuffle=False):
    ...
    if data_dir.rstrip('/').endswith('train'):
        lenth=batch_size*BATCHES_PER_EPOCH//2
        fake=list(range(0,219470))
        real=list(range(219470,219470+42690))
        if lenth>42690:real=real*(lenth//42690+1)
        random.shuffle(real)
        random.shuffle(fake)
        datasetind=real[:lenth*5//5]+fake[:lenth*5//5]
        random.shuffle(datasetind)
        #datasetind=torch.tensor(datasetind)
        #print(datasetind)
        print(dataset[datasetind[0]] [1])
        data_subset = torch.utils.data.Subset(dataset, datasetind)
    else:data_subset=dataset
...
def main():
    ...
    print(f"Train Dataloader: {len(train_loader)} batches of {BATCH_SIZE} images")
    print(f"Valid Dataloader: {len(valid_loader)} batches of {BATCH_SIZE} images") 

    model =PreTrainedBranch(output_features=1).to(device)
    model.to(device)
    ...
    fake_dir = os.path.join(valid_dir, "fake")

    # List all files and generate full paths
    ...
    valid_dataset = CustomDataset(file_paths, labels, valid_transform)
    valid_loader = DataLoader(valid_dataset, batch_size=128, shuffle=False, num_workers=8)

    eval_model = PreTrainedBranch(output_features=1).to(device)
    load_model(eval_model, "/home/nithira/sp_cup/keshawa/saved_states/combined_model_20250115_172001.pth", device)
